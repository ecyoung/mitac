        ...
        try:
            ...
            if algo_c == 'Linear Regression':
                ...
                try:
                    ...
                    # Gather dictionary for parameters and metrics
                    parameters = {'fit_intercept': fit_intercept, 'normalize': normalize, 'n_jobs': n_jobs}
                    metrics = {'score_train': score_train, 'mae_train': mae_train, 'score_test': score_test, 'mae_test': mae_test}
                ...
                try:
                    ...
                    # Gather dictionary for parameters and metrics.
                    parameters = {'n_estimators': n_estimators, 'criterion': criterion, 'max_depth': max_depth, 'min_sample_split': min_sample_split,
                                  'min_sample_split': min_sample_split, 'min_sample_leaf': min_sample_leaf, 'max_features': max_features,
                                  'bootstrap': bootstrap, 'max_samples': max_samples}
                    metrics = {'score_train': score_train, 'mae_train': mae_train, 'score_test': score_test,
                               'mae_test': mae_test}
                ...
                try:
                    ...
                    # Gather dictionary for parameters and metrics
                    parameters = {'kernel': kernel, 'degree': degree, 'gamma': gamma, 'coef0':coef0, 'C': C,
                                  'epsilon': epsilon, 'shrinking': shrinking, 'max_iter':max_iter}
                    metrics = {'score_train': score_train, 'mae_train': mae_train, 'score_test': score_test,
                               'mae_test': mae_test}
                    mlflow.set_experiment('SupportVectorRegression')
                ...
                try:
                    ...
                    # Gather dictionary for parameters and metrics
                    parameters = {'n_neighbors': n_neighbors, 'algorithm':algorithm, 'leaf_size':leaf_size, 'p': p}
                    metrics = {'score_train': score_train, 'mae_train': mae_train, 'score_test': score_test, 'mae_test': mae_test}
    ...
    try:
        if df is not None:
            ...
            with st.spinner(lang['training']):
                score_train, mae_train, score_test, mae_test, y_pred_train, y_pred_test, model, parameters, metrics = run_model(algo_chose, preprocessor)
