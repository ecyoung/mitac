    ...
    def run_model(algo_c, preprocessor):
        try:
            ...
            if algo_c == 'Logistic Regression':
                ...
                # Params into mlflow
                paramdist = {'penalty': penalty, 'dual': dual, 'C': C, 'solver': solver, 'multi_class': multi_class}
                # Metrics into mlflow
                metricsdict = {'f1': f1}
                mlflow.set_experiment(experiment_name='LogisticRegression')

                return f1, classi_rep, y_pred, model, paramdist, metricsdict
                ...
                # params into mlflow
                paramdist = {'n_estimators': n_estimators, 'max_depth': max_depth, 'criterion': criterion,
                             'min_samples_split': min_samples_split, 'min_samples_leaf': min_samples_leaf,
                             'max_features': max_features,'bootstrap': bootstrap, 'max_samples': max_samples}
                # Metrics into mlflow
                metricsdict = {'f1': f1}
                mlflow.set_experiment(experiment_name='RandomForestClassfier')
                ...
                # params into mlflow
                paramdist = {'C': C, 'kernel': kernel, 'degree': degree,
                             'coefo': coefo}
                # Metrics into mlflow
                metricsdict = {'f1': f1}
                # Get current time for run name
                mlflow.set_experiment(experiment_name='SupportVectorClassifier')
                ...
                # params into mlflow
                paramdist = {'n_neighbours': n_neighbours, 'weights': weights, 'algorithm': algorithm,
                             'leaf_size': leaf_size, 'p': p}
                # Metrics into mlflow
                metricsdict = {'f1': f1}
                mlflow.set_experiment(experiment_name='KNeighbors Classifier')
    ...
    if df is not None:
        try:
            ...
            with st.spinner(lang['training']):
                f1, classi_rep, y_pred, model, paramdist, metricsdict = run_model(algo_chose, preprocessor)
        ...
        if button:
            ...
            with mlflow.start_run(run_name=nameForRunName):
                mlflow.set_tag("mlflow.user", get_user())
